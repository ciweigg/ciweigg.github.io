;(function($,window,document,undefined){
	...
	EventFlow.prototype = {
		init : function(element,options){
			...
			var point = [];
  			...
  			$(window).scroll(function(event) {
  				...
  				for(var i =0;i<point.length;i++){ 3="" if(s+="" $(document).height()*2=""> point[i] ){
		...
		spliceHtml :function(element,options){
			...
			var middleLineI = '';
			var list = ''
			for(var i = 0;i<options.events.length;i++){ middlelinei="" +="<i class="first"></i>" +'<i=""><i></i><i></i><i></i><i></i><i></i><i></i><i></i><i></i>'
					+'<i class="last"></i>';
				if((i+1)%2 == 0){
...
</div></div></options.events.length;i++){></point.length;i++){>